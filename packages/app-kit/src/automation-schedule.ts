...
function offsetAt(atMs: number, timezone: string): number {
  ...
  return Date.UTC(parts.year, parts.month - 1, parts.day, parts.hour, parts.minute, parts.second) - atMs;
}
 ...
 * a gap stays explicit so callers can persist one terminal skipped fire. */
...
export function classifyAppAutomationOccurrence(input: Readonly<{
  ...
  if (input.occurrence.resolution.kind === 'dst_gap') {
    const occurrenceShape = parseLocal(input.occurrence.logical_local_date, input.occurrence.local_time).utc_shape;
    if (occurrenceShape <= localWallClockShape(input.eligible_after, input.occurrence.timezone)) {
...
/** Return the first real UTC fire strictly after both `now` and
 * `eligible_after`. DST gaps are skipped; the search is capped to 31 days. */
export function nextEligibleAppAutomationOccurrence(input: Readonly<{
  local_time: string;
  timezone: string;
  now: Date;
  eligible_after: Date;
  eligible_before?: Date;
  max_days?: number;
}>): AppAutomationOccurrence | null {
  const maxDays = Math.max(1, Math.min(31, input.max_days ?? 31));
  const reference = input.now > input.eligible_after ? input.now : input.eligible_after;
  let localDate = appAutomationLocalDate(reference, input.timezone);
  for (let index = 0; index < maxDays; index += 1, localDate = nextLogicalDate(localDate)) {
    const occurrence = resolveAppAutomationOccurrence({
      logical_local_date: localDate,
      local_time: input.local_time,
      timezone: input.timezone,
    });
    if (occurrence.resolution.kind !== 'resolved') continue;
    const resolvedAt = occurrence.resolution.resolved_at_utc;
    if (resolvedAt <= input.now || resolvedAt <= input.eligible_after) continue;
    if (input.eligible_before !== undefined && resolvedAt >= input.eligible_before) return null;
    return occurrence;
  }
  return null;
}
