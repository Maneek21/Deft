...
CREATE UNIQUE INDEX IF NOT EXISTS attention_item_user_dedupe_unique
  ON attention_items (org_id, user_id, dedupe_key);
CREATE INDEX IF NOT EXISTS attention_item_user_state_idx
  ON attention_items (org_id, user_id, state, last_event_at);
CREATE INDEX IF NOT EXISTS attention_item_user_lane_idx
  ON attention_items (org_id, user_id, lane, last_event_at);
CREATE INDEX IF NOT EXISTS attention_item_source_idx
  ON attention_items (org_id, source_type, source_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS attention_delivery_version_unique
  ON attention_deliveries (attention_item_id, channel, delivery_version);
