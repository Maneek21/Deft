...
const pages: Page[] = [
  ...
  { slug: 'decision-railway-deploy', title: 'Self-hosted Docker Deployment', type: 'decision', confidence: 1.0, scope: 'org',
    ...
    content: 'Decision: Deploy Deft as self-hosted Docker (not managed hosting).\n\nReason: Users run Deft on their own infrastructure. The API requires persistent connections (Socket.io) and background job processing, which are best managed via docker-compose on user infrastructure.\n\nStack: docker-compose with the Hono API, Next.js frontend, and PostgreSQL (pgvector:pg16). The application uses PostgreSQL for background jobs and Socket.io runs in-process for the single-instance launch topology.' },
  { slug: 'decision-bullmq-jobs', title: 'PostgreSQL for Background Jobs', type: 'decision', confidence: 1.0, scope: 'org',
    summary: 'PostgreSQL job_queue is the accepted launch queue; Redis and BullMQ are removed',
    content: 'Decision: Use the PostgreSQL job_queue for launch. It handles scheduling, claiming, retries, and stale-job recovery without a second datastore. Redis and BullMQ are not runtime dependencies.\n\nLaunch hardening focuses on idempotency, recurring-job uniqueness, renewable leases, retention, graceful shutdown, and queue health metrics. Reconsider another backend only when measured scale or multi-worker feature requirements justify a migration.' },
