...
import { readReleaseScope } from './resolve-release-scope.mjs';
...
const releaseScope = await readReleaseScope(process.env.RELEASE_SCOPE_PATH?.trim());
...
if (releaseScope.scope === 'hermes-certified') {
  ...
  const [certificate, evidence] = await Promise.all([readJson(certificatePath, 'Hermes employee certificate'), readJson(evidencePath, 'Hermes bundle evidence')]);
  ...
  expect(typeof manifest.hermes_compatibility === 'string' && manifest.hermes_compatibility.length > 0, 'verified bundle has no Hermes compatibility declaration');
  expect(contentDigest === `sha256:${manifest.content_digest?.value ?? ''}`, 'verified content digest does not match the bundle manifest');

  const testedRuntime = certificate.hermes?.tested_runtime;
  const declaredRuntime = manifest.hermes_tested;
  expect(certificate.hermes?.declared_compatibility === manifest.hermes_compatibility, 'certificate Hermes compatibility does not match the bundle');
  for (const field of ['distribution', 'version', 'repository', 'ref', 'commit']) {
    expect(typeof testedRuntime?.[field] === 'string' && testedRuntime[field].length > 0, `certificate Hermes tested runtime ${field} is missing`);
    expect(typeof declaredRuntime?.[field] === 'string' && declaredRuntime[field].length > 0, `bundle Hermes tested runtime ${field} is missing`);
    expect(testedRuntime?.[field] === declaredRuntime?.[field], `certificate Hermes tested runtime ${field} does not match the bundle`);
  }
  expect(/^[0-9a-f]{40}$/.test(testedRuntime.commit), 'certificate Hermes commit must be a full Git commit');
  expect(/^3\.11\.\d+$/.test(testedRuntime?.python_version ?? ''), 'certificate must record an observed Python 3.11 patch version');
  ...
  const fallbackAdapters = manifest.adapters.filter((adapter) => adapter.role === 'fallback').map(componentIdentity);
  expect(fallbackAdapters.length > 0, 'verified bundle must retain at least one fallback adapter');
  for (const [label, component] of [['default adapter', componentIdentity(defaultAdapter)], ...fallbackAdapters.map((component, index) => [`fallback adapter ${index + 1}`, component])]) {
    expect(
      Object.values(component).every((value) => typeof value === 'string' && value.length > 0),
  ...
  sameJson(componentIdentity(certificate.integration?.default_adapter), componentIdentity(defaultAdapter), 'certificate default adapter');
  sameJson((certificate.integration?.fallback_adapters ?? []).map(componentIdentity), fallbackAdapters, 'certificate fallback adapters');
  expect(certificate.integration?.manifest_sha256 === manifestDigest, 'certificate manifest digest does not match the downloaded bundle');
  expect(certificate.integration?.content_sha256 === contentDigest, 'certificate content digest does not match the downloaded bundle');

  expect(Array.isArray(certificate.passes) && certificate.passes.length === certificate.consecutive_passes, 'certificate pass count does not match consecutive_passes');
  for (const [index, pass] of certificate.passes.entries()) {
    expect(pass?.pass === index + 1, `certificate pass ${index + 1} is out of order`);
    expect(typeof pass.completed_at === 'string' && !Number.isNaN(Date.parse(pass.completed_at)), `certificate pass ${index + 1} has no valid completion time`);
    sameJson(
      pass.suites,
      expectedSuites.map((suite) => ({ ...suite, result: 'passed' })),
      `certificate pass ${index + 1} suite evidence`,
    );
    expect(pass.bundle?.manifest_sha256 === manifestDigest, `certificate pass ${index + 1} manifest digest does not match the downloaded bundle`);
    expect(pass.bundle?.content_sha256 === contentDigest, `certificate pass ${index + 1} content digest does not match the downloaded bundle`);
  }
