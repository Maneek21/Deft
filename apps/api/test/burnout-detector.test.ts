/**
 * burnout-detector service unit tests — authorship overload signal.
 *
...
test('1. detected:true when recent 14d count is >3x baseline', async () => {
  ...
  try {
    ...
    assert.equal(signal.weight, 0.15, 'signal weight should be 0.15');
...
test('2. detected:false when recent count is within normal range', async () => {
  ...
  try {
    ...
    assert.equal(signal.weight, 0.15);
    assert.equal(
      signal.detected,
      false,
