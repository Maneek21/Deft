...
test('fresh Hermes setup uses the native platform and direct HTTP MCP only', () => {
  ...
  try {
    ...
    assert.equal(setup.integration_version, '0.4.0');
    assert.equal(setup.mcp_endpoint_url, 'https://deft.example/api/mcp/v1');
    ...
    assert.match(setup.config_snippet, /mcp_servers:[\s\S]*url: https:\/\/deft\.example\/api\/mcp\/v1/);
