...
export type SkillAgentConfig = {
  ...
  /**
   * Env vars the skill requires at install time. Pre-deploy flow matches
   * each key against connected_accounts (OAuth) first.
   */
  requires_env?: string[];
  /**
   * MCP server the skill wires the agent sidecar to. Declared by
   * `deft-mcp-client` so BYOA agents register Deft's MCP server at boot;
   * other skills may use this too in the future.
   */
  mcp_servers?: Array<{
    name: string;
