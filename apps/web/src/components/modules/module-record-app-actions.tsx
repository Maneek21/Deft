...
export function ModuleRecordAppActions({ resourceRef, enabled = true }: { resourceRef: ResourceRef; enabled?: boolean }) {
  ...
  const [selectedAction, setSelectedAction] = useState<AppActionItem | null>(null);
  ...
  const selectionReady = Boolean(resolved && resolved.inputs.every((input) => input.kind !== 'selected_relation_field' || Boolean(selections[input.inputKey])));
  if (!enabled || !APPS_ENABLED) return null;
  if (actionsState.isLoading) return <div role="status" className="flex min-h-8 items-center gap-2 px-1 text-[11px]" style={{ color: 'var(--on-surface-variant)' }}><Loader2 size={13} className="animate-spin" aria-hidden="true" /> Checking App actions…</div>;
  if (!actionsState.isLoading && !actionsState.error && actionsState.actions.length === 0) return null;
  ...
  return (
    <section className="rounded-xl p-3.5" style={{ background: 'var(--surface-container-low)', border: '1px solid var(--ghost-border)' }} aria-label="App actions">
      <div className="flex items-start gap-2.5"><span className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg" style={{ background: 'var(--bg-active)', color: 'var(--primary)' }}><Play size={15} /></span><div><h2 className="text-[0.8125rem] font-semibold">App actions</h2><p className="mt-0.5 text-[0.6875rem]" style={{ color: 'var(--on-surface-variant)' }}>Actions available for this record.</p></div></div>
      {actionsState.error ? <p role="alert" className="mt-3 text-[0.6875rem]" style={{ color: 'var(--error)' }}>{actionsState.error instanceof Error ? actionsState.error.message : 'App actions are unavailable.'} <button type="button" className="min-h-11 underline" onClick={() => void actionsState.mutate()}>Retry</button></p>
          : actionsState.actions.length === 0 ? <div className="mt-3 space-y-2 text-xs"><p>No connected action is available for this record.</p><p style={{ color: 'var(--on-surface-variant)' }}>{canManageAutomations ? 'Review the installed App, its connector and approved permissions in Apps settings.' : 'Ask a workspace owner or admin to review the App setup and permissions.'}</p>{canManageAutomations && <Link className="inline-flex min-h-11 items-center underline" href="/settings/apps">Open Apps settings</Link>}<button type="button" className="ml-3 min-h-11 underline" onClick={() => void actionsState.mutate()}>Refresh actions</button></div>
      ...
      <AppDialog
        ...
        description="Choose the linked record, review the recipient and content, then confirm the action."
        width={520}
        footer={<ActionFooter busy={busy} selectionReady={selectionReady} resolved={resolved} prepared={prepared} run={run} canManageAutomations={canManageAutomations} automationSelectionReady={automationSelectionReady} automationMode={automationMode} automationReview={automationReview} automationCreated={automationCreated} onClose={close} onPrepare={() => void prepare()} onInvoke={() => void invoke()} onBeginAutomation={() => { setAutomationMode(true); setError(null); }} onReviewAutomation={() => void reviewAutomation()} onConfirmAutomation={() => void confirmAutomation()} />}
      >
        ...
        {selectedAction && !prepared && !run && !automationMode && <button type="button" className="mb-3 min-h-11 text-xs underline" disabled={Boolean(busy)} onClick={() => void begin(selectedAction)}>Refresh action inputs</button>}
        ...
        {resolved && !prepared && !run && !automationMode && <div className="space-y-3">{resolved.inputs.map((input) => {
          ...
          if (input.kind === 'selected_relation_field') return <label key={input.inputKey} className="block text-xs font-medium">{humanize(input.inputKey)}<select className="mt-1 min-h-11 w-full rounded-lg px-3 text-xs" style={{ background: 'var(--surface-container-low)', border: '1px solid var(--outline-variant)' }} value={selections[input.inputKey] ? resourceRefKey(selections[input.inputKey]) : ''} onChange={(event) => {
            ...
            setError(null);
          }}><option value="">Choose a related resource</option>{input.options.map((option) => <option key={resourceRefKey(option.ref)} value={resourceRefKey(option.ref)}>{option.label}</option>)}</select>{input.options.length === 0 && <span className="mt-1 block text-[11px]" style={{ color: 'var(--error)' }}>No authorized related record is available. Add a link in {humanize(input.relationKey)}, then refresh action inputs.</span>}</label>;
        ...
        {prepared && !run && <div className="space-y-4"><div role="status" className="rounded-lg px-3 py-2.5" style={{ background: 'var(--warning-subtle)' }}><p className="text-xs font-semibold">Draft ready · not submitted</p><p className="mt-1 text-[11px]" style={{ color: 'var(--on-surface-variant)' }}>Review this sandbox message before submitting it for approval.</p></div>{prepared.reviewFields ? <div className="space-y-3"><ReadOnlyInput label="To" value={prepared.reviewFields.to} /><ReadOnlyInput label="Subject" value={prepared.reviewFields.subject} /><div className="rounded-lg p-3" style={{ background: 'var(--surface-container-high)' }}><h3 className="text-xs font-medium">Message</h3><p className="mt-2 max-h-64 overflow-auto whitespace-pre-wrap break-words text-sm">{prepared.reviewFields.bodyText}</p></div></div> : <p role="alert" className="text-xs">Message review is unavailable. Close this dialog and refresh before continuing.</p>}<details><summary className="min-h-11 cursor-pointer text-xs">Action details</summary><SafePreview preview={prepared.safePreview} /></details></div>}
...
function ActionFooter({ busy, selectionReady, resolved, prepared, run, canManageAutomations, automationSelectionReady, automationMode, automationReview, automationCreated, onClose, onPrepare, onInvoke, onBeginAutomation, onReviewAutomation, onConfirmAutomation }: {
  ...
  return <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
    <button type="button" className="deft-pill min-h-11" style={{ minHeight: 44 }} disabled={Boolean(busy)} onClick={onClose}>{done ? 'Done' : 'Cancel'}</button>
    {canSchedule && <button type="button" className="deft-pill min-h-11" style={{ minHeight: 44 }} disabled={Boolean(busy)} onClick={onBeginAutomation}><Clock3 size={13} /> Schedule daily</button>}
    {resolved && !prepared && !run && !automationMode && <button type="button" className="deft-pill min-h-11 text-white" style={{ background: 'var(--primary-container)', color: 'white', minHeight: 44 }} disabled={Boolean(busy) || !selectionReady} onClick={onPrepare}>{busy === 'prepare' && <Loader2 size={13} className="animate-spin" />} Review action</button>}
    {automationMode && !automationReview && !automationCreated && <button type="button" className="deft-pill min-h-11 text-white" style={{ background: 'var(--primary-container)', color: 'white', minHeight: 44 }} disabled={Boolean(busy)} onClick={onReviewAutomation}>{busy === 'automation-review' && <Loader2 size={13} className="animate-spin" />} Review schedule</button>}
    {automationReview && !automationCreated && <button type="button" className="deft-pill min-h-11 text-white" style={{ background: 'var(--primary-container)', color: 'white', minHeight: 44 }} disabled={Boolean(busy)} onClick={onConfirmAutomation}>{busy === 'automation-create' && <Loader2 size={13} className="animate-spin" />} Approve and create</button>}
    {prepared && !run && <button type="button" className="deft-pill min-h-11 text-white" style={{ background: 'var(--primary-container)', color: 'white', minHeight: 44 }} disabled={Boolean(busy) || !prepared.reviewFields} onClick={onInvoke}>{busy === 'invoke' && <Loader2 size={13} className="animate-spin" />} Run in sandbox</button>}
  </div>;
}

function AutomationResourcePin({ label, pin }: { label: string; pin: AppAutomationReview['placement'] }) {
  const { resourceRef } = pin;
  return <div className="rounded-lg px-3 py-2.5" style={{ background: 'var(--surface-container-high)' }}><p className="text-[11px] font-semibold">{label}</p><code className="mt-1 block break-all text-[11px]">{resourceRef.providerKind}:{resourceRef.providerInstanceId}:{resourceRef.resourceType}:{resourceRef.resourceId}</code><code className="mt-1 block break-all text-[10px]" style={{ color: 'var(--on-surface-variant)' }}>revision {pin.revision} · {pin.contentDigest}</code></div>;
}

function ReadOnlyInput({ label, value }: { label: string; value: string }) {
  return <div className="rounded-lg px-3 py-2.5" style={{ background: 'var(--surface-container-high)' }}><p className="text-[11px] font-medium">{label}</p><p className="mt-0.5 whitespace-pre-wrap break-words text-xs" style={{ color: 'var(--on-surface-variant)' }}>{value}</p></div>;
}

function SafePreview({ preview }: { preview: AppActionPrepared['safePreview'] }) {
  return <div className="space-y-3"><div className="flex items-start gap-2 rounded-xl p-3" style={{ background: 'rgba(48,164,108,.10)' }}><ShieldCheck size={16} className="mt-0.5 flex-shrink-0" style={{ color: 'var(--status-green)' }} /><div><h3 className="text-sm font-semibold">{preview.title}</h3>{preview.summary && <p className="mt-1 text-xs leading-relaxed" style={{ color: 'var(--on-surface-variant)' }}>{preview.summary}</p>}</div></div>{preview.resourceLabels.length > 0 && <ReadOnlyInput label="Resources" value={preview.resourceLabels.join(', ')} />}{Object.keys(preview.fields).length > 0 && <dl className="space-y-2">{Object.entries(preview.fields).map(([key, value]) => <div key={key} className="rounded-lg px-3 py-2" style={{ background: 'var(--surface-container-high)' }}><dt className="text-[10px] font-semibold uppercase" style={{ color: 'var(--on-surface-variant)' }}>{humanize(key)}</dt><dd className="mt-1 break-words text-xs">{formatJsonValue(value)}</dd></div>)}</dl>}<p className="text-[11px]" style={{ color: 'var(--on-surface-variant)' }}>This preview contains safe metadata only. Deft revalidates authority before running the action.</p></div>;
}
