...
export const AGENT_RUNTIMES: AgentRuntime[] = [
  ...
  },
  {
    ...
    defaultWakeMode: 'external_chat',
    transports: ['Native Deft platform adapter', 'MCP streamable HTTP'],
    setupNotes: [
      'Assume Hermes is already running with a model and personality.',
      'Install the matched Deft integration bundle and enable only its native deft-platform adapter.',
      'Attach Deft directly as an HTTP MCP server named "deft" and preserve Hermes-owned identity prompts.',
    ...
    caveats: [
      ...
      'The legacy Node bridge is rollback-only; never run it alongside deft-platform for the same employee.',
    ],
    ownsIdentity: true,
  },
  {
